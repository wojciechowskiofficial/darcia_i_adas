...
public class Searcher
{
    ...
    public static void main(String args[]) throws IOException, ParseException {
        ...
        // Boolean query
        {
            ...
            BooleanClause mammal = new BooleanClause(tq1, BooleanClause.Occur.SHOULD);
            BooleanClause bird = new BooleanClause(tq2, BooleanClause.Occur.SHOULD);
            BooleanQuery.Builder booleanQuery = new BooleanQuery.Builder();
            booleanQuery.add(mammal);
            booleanQuery.add(bird);
            booleanQuery.setMinimumNumberShouldMatch(1);
            BooleanQuery buildedBooleanQuery = booleanQuery.build();
            printResultsForQuery(indexSearcher, buildedBooleanQuery);
            // --------------------------------------
        }
        ...
        // Use IntPoint.newRangeQuery.
        {
            ...
            printResultsForQuery(indexSearcher,
                    IntPoint.newRangeQuery(Constants.filesize_int,0,999));
            // --------------------------------------
        }
    ...
    private static void printResultsForQuery(IndexSearcher indexSearcher, Query q) throws IOException {
        ...
        for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
            System.out.println(scoreDoc.score
                    + ": " + indexSearcher.doc(scoreDoc.doc).getField(Constants.filename)
                    + " (Id=" + scoreDoc.doc + ") (Content="
                    + indexSearcher.doc(scoreDoc.doc).getField(Constants.content)
                    + ") (Size=" + indexSearcher.doc(scoreDoc.doc).getField(Constants.filesize)+ ")");
        }        // --------------------------------
    }
